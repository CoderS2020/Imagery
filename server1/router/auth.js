...
router.get('/', (req, res) => {
    ...
    res.send(`Hello from server in auth.js`);
});
...
router.post('/register', async (req, res) => {
    ...
    const { firstname, lastname, email, password, age, creamzoId } = req.body;
    ...
    try {
        ...
        const token1 = jwt.sign({ firstname, lastname, email, password, age, fullName, creamzoId }, process.env.SECRET_KEY, { expiresIn: '20m' });

        const mailOptions = {
            from: process.env.EMAIL,
            to: email,
            subject: "Creamzo Authentication!",
            html: `
            <h2>Please click on the given link to activate your account.</h2>
            <a href='http://localhost:3000/activate/${token1}'>Activate</a>
        ...
        // const user = new User({ firstname, lastname, email, password, age, fullName, creamzoId });
    ...
    try {
        ...
        if (token1) {
            jwt.verify(token1, process.env.SECRET_KEY, function (err, decodedToken) {
                ...
                const { firstname, lastname, email, password, age, fullName, creamzoId } = decodedToken;
                
                const userExist = User.findOne({ firstname: firstname });
                // console.log(userExist);
                if (userExist) {
                    const userExist = User.findOneAndUpdate({firstname, lastname, email, password, age, fullName, creamzoId});
...
router.put('/forgotPassword', (req,res)=>{
    ...
    User.findOne({email},(err,user)=>{
        ...
        const token2=jwt.sign({_id:user._id},process.env.RESET_KEY, { expiresIn: '20m' });
      
        const mailOptions = {
            from: process.env.EMAIL,
            to: email,
            subject: "Creamzo Account Password Reset",
            html: `
            <h2>Please click on the given link to reset your password.</h2>
            <a href='http://localhost:3000/resetpassword/${token2}'>Reset Password</a>
        ...
        return user.updateOne({resetLink:token2},function(err,success){
            ...
            return res.status(200).json("Email has been sent, kindly follow the instructions");
        })
...
router.post('/googlesignup', async (req, res) => {
    ...
    try {
        const response = await client.verifyIdToken({ idToken: tokenId, audience: process.env.OAUTH_CLIENT_ID });

        const { email_verified, name, email, given_name, family_name, picture } = response.payload;
        const profilepic = picture;
        const fullName = name;
        let creamzoId = (String(given_name) + time).trim();
        ...
        if (email_verified) {
            ...
            if (userLogin) {
                const isMatch = await bcrypt.compare(email + process.env.ANYTHING, userLogin.password);
                ...
                const user = new User({ firstname, lastname, email, password, age, fullName, creamzoId, profilepic });

                await user.save();

                res.status(201).json({ messageSuccess: "User registered successfully,Please Continue with Google again" });
...
router.post('/contact', Authenticate, async (req, res) => {
    try {
        ...
        if (userContact) {
            const userMessage = await userContact.addMessage(name, email, country, message, rating);
...
router.post('/updateprofile', async (req, res) => {
    try {
        const { firstname, lastname, email, about, location, gender, profilepic } = req.body;
        const fullName = firstname + " " + lastname;
        const editDetails = await User.findOneAndUpdate({ email: email }, { firstname, lastname, about, location, gender, profilepic, fullName }, { new: true });
        // const userContact=await User.findOne({_id:req.userId});
        if (editDetails) {
...
router.post('/saveimage', async (req, res) => {
    ...
    const { author, title, description, destination, tags, pin_sizes, url, email, timeofUpload, idUploader } = req.body;
    ...
    try {
        ...
        const imageInfo = new Image({ author, title, description, destination, tags, pin_sizes, url, email, timeofUpload, idUploader });
...
router.post('/Like', async (req, res) => {
    ...
    try {
        ...
        const imageData = await Image.findByIdAndUpdate(imageid, { $push: { likes: userId } }, { new: true });
...
router.post('/unLike', async (req, res) => {
    ...
    try {
        ...
        const imageData = await Image.findByIdAndUpdate(imageid, { $pull: { likes: userId } }, { new: true });
...
router.post('/addImageCollection', async (req, res) => {
    ...
    try {
        ...
        const userData = await User.findByIdAndUpdate(userId, { $push: { addedImages: imageid } }, { new: true });
...
router.post('/removeImageCollection', async (req, res) => {
    ...
    try {
        ...
        const userData = await User.findByIdAndUpdate(userId, { $pull: { addedImages: imageid } }, { new: true });
...
router.post('/downloadCount', async (req, res) => {
    ...
    try {
        ...
        const imageData = await Image.findByIdAndUpdate(imageid, { $push: { download: userId } }, { new: true });
...
router.post('/shareCount', async (req, res) => {
    ...
    try {
        ...
        const imageData = await Image.findByIdAndUpdate(imageid, { $push: { share: userId } }, { new: true });
...
router.post('/viewCount', async (req, res) => {
    ...
    try {
        ...
        const imageData = await Image.findByIdAndUpdate(imageid, { $push: { view: userId } }, { new: true });
            ...
            if (err) {
                ...
                User.findById(authorId, function (err, docs1) {
                    if (err) {
                        ...
                        User.findByIdAndUpdate(authorId, { $push: { followers: { Sname, Spic, Sid } } }, { new: true }, (err, result) => {
                            if (err) {
                                return res.status(422).json({ error: err });
                            }

                            User.findByIdAndUpdate(selfId, { $push: { following: { Aname, Apic, Aid } } }, { new: true })
                                .then(result => {
                                    res.json(result)
                                }).catch(err => {
                                    return res.status(422).json({ error: err });
            ...
            if (err) {
                ...
                User.findById(authorId, function (err, docs1) {
                    if (err) {
                        ...
                        User.findByIdAndUpdate(authorId, { $pull: { followers: { Sname, Spic, Sid } } }, { new: true }, (err, result) => {
                            if (err) {
                                return res.status(422).json({ error: err });
                            }

                            User.findByIdAndUpdate(selfId, { $pull: { following: { Aname, Apic, Aid } } }, { new: true })
                                .then(result => {
                                    res.json(result)
                                }).catch(err => {
                                    return res.status(422).json({ error: err });
